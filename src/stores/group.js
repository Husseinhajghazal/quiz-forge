...
export function addGroup(data) {
  if (!data.points) return addToast("يجب عليك أدخال علامة المجموعة", "error");

  if (data.type === "اختيار من متعدد" && !data.choicesCount)
    return addToast("يجب عليك أدخال عدد الخيارات", "error");

  if (
    data.type === "اختيار من متعدد" &&
    (data.choicesCount < 3 || data.choicesCount > 5)
  )
    return addToast("عدد الخيارات يجب أن يكون بين 3 و 5 خيارات فقط", "error");

  if (data.points < 0)
    return addToast("يجب عليك أدخال علامة المجموعة", "error");
